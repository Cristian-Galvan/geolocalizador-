...
@app.get("/visited", response_model=List[schemas.VisitedPlaceResponse])
async def get_ratings(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    if not current_user:
        raise HTTPException(status_code=401, detail="No autenticado")
    
    return db.query(models.VisitedPlace).filter(
        models.VisitedPlace.user_id == current_user.username
    ).order_by(models.VisitedPlace.timestamp.desc()).all()

@app.delete("/visited/{item_id}")
async def delete_rating(item_id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    if not current_user:
        raise HTTPException(status_code=401, detail="No autenticado")
